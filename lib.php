 ...
 */
function callback_topmas_uses_sections() {
    return true;
}
 ...
 * @return bool Returns true
 */
function callback_topmas_load_content(&$navigation, $course, $coursenode) {
 ...
 * @return string
 */
function callback_topmas_definition() {
 ...
 * @return string
 */
function callback_topmas_request_key() {
    return 'topic';
}

function callback_topmas_get_section_name($course, $section) {
    // We can't add a node without any text
    if (!empty($section->name)) {
        return format_string($section->name, true, array('context' => get_context_instance(CONTEXT_COURSE, $course->id)));
    } else if ($section->section == 0) {
        return get_string('section0name', 'format_topmas');
    } else {
 ...
 * @return stdClass
 */
function callback_topmas_ajax_support() {
